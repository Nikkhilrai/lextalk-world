...
function RegistrationModal({ isOpen, onClose, couponApplied, discountPct, appliedCode }: {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const options = {
                ...
                currency: "INR",
                name: "LexTalk World",
                description: "Corporate Counsel Exclusive Pass — Mumbai 2026",
                ...
                handler: async (response: any) => {
                    ...
                    if (verifyData.success) {
                        setTimeout(() => { onClose(); router.push(`/mumbai-delegate-confirmation-2026?regId=${orderData.registrationId}`); }, 3000);
                    } else {
    ...
    const handleFreeRegistration = async () => {
        ...
        try {
            ...
            setTimeout(() => {
                onClose();
                router.push(`/mumbai-delegate-confirmation-2026?regId=${data.id}`);
            }, 3000);
        } catch (err: any) {
            setError(err.message || "Something went wrong");
            setIsProcessing(false);
                ...
                <div className="fixed inset-0 z-[100000] flex items-end sm:items-start justify-center p-0 sm:p-4 sm:pt-24 overflow-y-auto">
                    ...
                    >
                        <div className="relative flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-amber-50/30">
                            <div>
                                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-amber-600 mb-0.5">Corporate Counsel · Mumbai 2026</p>
                                ...
                                {step === 1 ? (
                                    <motion.form key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <PhoneInput value={formData.phone} onChange={val => setFormData(p => ({ ...p, phone: val }))} id="mcc-phone" variant="pill" />
                                        </div>
                                        <div className="grid grid-cols-2 gap-3">
                                            {[
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <CountrySelect value={formData.country} onChange={val => setFormData(p => ({ ...p, country: val }))} id="mcc-country" variant="pill" />
                                        </div>
                                        {error && (
...
function PassCard({ couponApplied, discountPct, appliedCode, couponInput, setCouponInput, couponError, setCouponError, onApplyCoupon, onRemoveCoupon, onRegister, validatingCoupon }: {
    couponApplied: boolean; discountPct: number; appliedCode: string;
    couponInput: string; setCouponInput: (v: string) => void;
    couponError: string; setCouponError: (v: string) => void;
    onApplyCoupon: () => void; onRemoveCoupon: () => void; onRegister: () => void;
                ...
                <div className="p-6">
                    ...
                    {!couponApplied && (
                        <div className="flex items-center gap-2.5 px-3.5 py-2.5 bg-amber-500/8 border border-amber-500/20 rounded-xl mb-5">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <button
                                    onClick={() => { setCouponInput("EARLY30"); setCouponError(""); }}
                    ...
                    <div className="mb-5">
                        <div className="flex items-baseline gap-2 mb-1">
                            {couponApplied && <span className="text-lg font-bold text-slate-600 line-through">₹{ORIGINAL_PRICE.toLocaleString("en-IN")}</span>}
                            {isFree ? (
                        ...
                        {!isFree && (
                            <p className="text-slate-500 text-[11px]">Total: <span className="text-slate-300 font-semibold">₹{total.toLocaleString("en-IN")}</span> incl. 18% GST</p>
                        )}
                        {couponApplied && (
...
/* ── Main Page ── */
export default function MumbaiCorporateCounselPass() {
    ...
    const handleApplyCoupon = async () => {
        ...
        try {
            const res = await fetch("/api/delegate-coupons/validate", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ code, passType: "corporate-counsel", conferenceSlug: "mumbai-2026" }),
            });
            const data = await res.json();
            if (data.valid) {
                        ...
                        <div className="lg:pt-8">
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: loaded ? 1 : 0, y: loaded ? 0 : 20 }} transition={{ duration: 0.6, delay: 0.1 }}>
                                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber-500/10 border border-amber-500/25 rounded-full mb-6">
                                    ...
                                    <span className="text-amber-400 text-[10px] font-bold uppercase tracking-[0.25em]">LexTalk World · Mumbai 2026</span>
                                </div>
                            </motion.div>
                            ...
                            >
                                India&apos;s premier in-house legal summit. Join 300+ senior legal leaders, GCs, and CLOs at Mumbai&apos;s most exclusive legal conference.
                            </motion.p>

                            {/* Audience */}
                            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: loaded ? 1 : 0, y: loaded ? 0 : 16 }} transition={{ duration: 0.6, delay: 0.4 }} className="mb-8">
                                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-3">For</p>
                                <div className="flex flex-wrap gap-2">
                                    {AUDIENCE.map(tag => (
                                        <span key={tag} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-slate-400 text-[11px] font-medium">{tag}</span>
                                    ))}
                                </div>
                            </motion.div>
