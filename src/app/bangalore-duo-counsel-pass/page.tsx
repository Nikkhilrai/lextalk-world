...
import {
    ...
    ShieldCheck, Users, Mic, Wifi, Coffee, Award,
    BookOpen, Star, X, Calendar, MapPin, Zap,
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

declare global { interface Window { Razorpay: any; } }

// ₹15,000 GST inclusive — 2 passes
const TOTAL_PRICE = 15000;
const PASSES = 2;

const FEATURES = [
    { icon: Users,    text: "2 entry passes included" },
...
function RegistrationModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const steps = [
        ...
        "Generating your unique conference tickets…",
        ...
        try {
            const res = await fetch("/api/delegate-registration/save-lead", {
                ...
                body: JSON.stringify({
                    ...
                    passType: "corporate-counsel-duo",
                    passCategory: "duo",
    ...
    const handlePayment = async () => {
        ...
        try {
            const orderRes = await fetch("/api/delegate-registration/create-order", {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    amount: TOTAL_PRICE, currency: "INR",
                    passType: "corporate-counsel-duo", passCategory: "duo",
                    paymentType: "india", customerDetails: formData,
                    conferenceSlug: "bangalore-2026",
                    originalPrice: TOTAL_PRICE, discountedPrice: TOTAL_PRICE,
                    baseInrPrice: TOTAL_PRICE, baseUsdPrice: TOTAL_PRICE,
                    registrationId,
                }),
            });
            ...
            const options = {
                ...
                currency: "INR",
                name: "LexTalk World",
                description: "Corporate Counsel Duo Pass (2 Passes) — Bangalore 2026",
                ...
                <div className="fixed inset-0 z-[100000] flex items-end sm:items-start justify-center p-0 sm:p-4 sm:pt-24 overflow-y-auto">
                    ...
                    >
                        <div className="relative flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-amber-50/30">
                            <div>
                                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-amber-600 mb-0.5">Duo Pass · Bangalore 2026</p>
                                ...
                                {step === 1 ? (
                                    <motion.form key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <PhoneInput value={formData.phone} onChange={val => setFormData(p => ({ ...p, phone: val }))} id="duo-phone" variant="pill" />
                                        </div>
                                        <div className="grid grid-cols-2 gap-3">
                                            {[
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <CountrySelect value={formData.country} onChange={val => setFormData(p => ({ ...p, country: val }))} id="duo-country" variant="pill" />
                                        </div>
                                        {error && (
                                    ...
                                    <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                                        <div className="p-4 bg-gradient-to-br from-slate-50 to-amber-50/30 rounded-2xl border border-slate-100">
                                            ...
                                            <div className="space-y-1.5 text-sm">
                                                {[
                                                    { label: "Pass", value: "Corporate Counsel Duo (2 Passes)" },
                                        ...
                                        <div className="p-4 bg-amber-50/60 border border-amber-100 rounded-2xl space-y-2">
                                            ...
                                            <div className="flex justify-between text-sm">
                                                <span className="text-slate-500">2 Passes Bundle</span>
                                                <span className="font-semibold text-slate-900">₹{TOTAL_PRICE.toLocaleString("en-IN")}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                            ...
                                            <div className="flex justify-between text-base font-black border-t border-amber-200 pt-2 mt-1">
                                                ...
                                                <span className="text-slate-900">₹{TOTAL_PRICE.toLocaleString("en-IN")}</span>
                                            </div>
                                        </div>
                                        {error && (
                                            <div className="flex items-center gap-2 p-3 bg-red-50 text-red-600 text-xs rounded-xl border border-red-100">
                                        ...
                                        >
                                            <span className="text-[10px] font-black uppercase tracking-widest mb-1 text-slate-900/60">Pay via Razorpay · 2 Passes</span>
                                            <span className="text-2xl font-black">₹{TOTAL_PRICE.toLocaleString("en-IN")}</span>
                                            <span className="text-[9px] mt-1 text-slate-900/50">GST inclusive · ₹{(TOTAL_PRICE / PASSES).toLocaleString("en-IN")} per pass</span>
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/15 border border-amber-500/25 rounded-full">
                            ...
                            <span className="text-amber-400 text-[9px] font-black uppercase tracking-[0.2em]">Duo Pass · 2 Seats</span>
                        </div>
                    ...
                    <h2 className="font-serif text-2xl font-bold text-white leading-tight mb-1">Duo Pass</h2>
                    <p className="text-slate-500 text-xs mb-3">2 passes for your team</p>
                    ...
                    <div className="mb-5">
                        <div className="flex items-baseline gap-2 mb-1">
                            <span className="text-4xl font-black text-white leading-none">₹{TOTAL_PRICE.toLocaleString("en-IN")}</span>
                            <span className="text-slate-400 text-xs">incl. GST</span>
                        </div>
                        <p className="text-slate-500 text-[11px]">
                            ₹{(TOTAL_PRICE / PASSES).toLocaleString("en-IN")} per pass · {PASSES} passes included
                        </p>
                    </div>
...
/* ── Main Page ── */
export default function BangaloreDuoCounselPass() {
                ...
                <div className="container mx-auto px-4 max-w-6xl relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                        ...
                        <div className="lg:pt-8">
                            ...
                            {/* Audience tags */}
                            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: loaded ? 1 : 0, y: loaded ? 0 : 16 }} transition={{ duration: 0.6, delay: 0.4 }}
                                className="mb-8"
                            >
                                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-3">For</p>
                                <div className="flex flex-wrap gap-2">
                                    {["In-House Counsel", "Data Privacy Officers", "Compliance Officers", "Legal Managers", "Cybersecurity Officers"].map(tag => (
                                        <span key={tag} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-slate-400 text-[11px] font-medium">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {/* Event details */}
                            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: loaded ? 1 : 0, y: loaded ? 0 : 16 }} transition={{ duration: 0.6, delay: 0.5 }}
                                className="flex flex-wrap gap-4"
                            >
                                {[
                                    { icon: Calendar, text: "June 11, Thursday, 2026" },
                                    { icon: MapPin,   text: "Radisson Blu Atria, Bangalore" },
